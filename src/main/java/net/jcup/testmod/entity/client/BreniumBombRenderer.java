...
public class BreniumBombRenderer extends EntityRenderer<BreniumBombEntity> {
    private static final Identifier TEXTURE = new Identifier(JcupsMod.MOD_ID, "textures/entity/brenium_bomb.png");
    private EntityModel<BreniumBombEntity> model;

    public BreniumBombRenderer(EntityRendererFactory.Context context) {
        super(context);
        model = new BreniumBombModel<>();
    }

    @Override
    public Identifier getTexture(BreniumBombEntity entity) {
        return TEXTURE;
    }

    public void render(BreniumBombEntity entity, float yaw, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light) {
        matrices.push();
